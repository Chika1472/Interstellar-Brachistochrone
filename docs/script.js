...
const CHEMICAL_ENERGY_PER_TON = 43e9;
const HIROSHIMA_YIELD_J = 6.3e13;
const WORLD_ANNUAL_ENERGY_J = 6.0e20;
...
const TARGETS = RAW_TARGETS.map((item) => {
    ...
    return { name: item.name, distanceKm };
...
function updateWarning(element, message = "", color = "#f1a355") {
    ...
    if (message) {
        element.classList.toggle('error', color === '#ff5d5d');
    } else {
...
function populateTargets() {
    ...
    if (TARGET_LOOKUP.has("Proxima Centauri")) {
        destinationSelect.value = "Proxima Centauri";
    }
}
...
function formatDurationExtended(seconds) {
    ...
    if (seconds >= SECONDS_IN_YEAR) {
        ...
        if (years >= 1e6) return `${(years / 1e6).toLocaleString(undefined, { maximumFractionDigits: 2 })} million years`;
        if (years >= 1e3) return `${years.toLocaleString(undefined, { maximumFractionDigits: 0 })} years`;
...
function formatRelativeSpeed(relativeToC, velocityMs) {
    ...
    return `${relativeToC.toLocaleString(undefined, { minimumFractionDigits: 3, maximumFractionDigits: 3 })} c (${(velocityMs / 1000).toLocaleString(undefined, { maximumFractionDigits: 2 })} km/s)`;
}
...
function updateAccelerationFromManual() {
    ...
    if (!Number.isFinite(manualAcceleration) || manualAcceleration <= 0) {
        setAccelerationWarning("Manual acceleration must be a positive number.", "#ff5d5d");
    ...
    const clampedG = Math.min(Math.max(manualG, parseFloat(accelerationRange.min)), parseFloat(accelerationRange.max));
...
function computeTrajectory() {
    ...
    highlightSpeed.textContent = formatRelativeSpeed(relativeToC, peakVelocity);
    ...
    detailPeakVelocity.textContent = formatVelocity(peakVelocity);
